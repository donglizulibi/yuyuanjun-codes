{
  ...
  function funcString(arg: string) {
    return arg;
  }
  function funcNumber(arg: number) {
    return arg;
  }
  function funcBoolean(arg: boolean) {
    return arg;
  }
  ...
  function funcGeneric<T>(arg: T): T {
    return arg;
  }

  // 在具体传递参数的时候，可以根据参数的类型，手动定义泛型的类型
  let str = funcGeneric<string>("houdunren");
  console.log(str);

  // 当然如果不去手动定义，具备泛型的函数也可以自动推断出函数返回值的类型
  let num = funcGeneric(123);
  let bool = funcGeneric(true);
  console.log(num);
  console.log(bool);
}
